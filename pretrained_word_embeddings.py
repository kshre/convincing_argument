...
for csv_file in sorted(os.listdir(ARGUMENT_DATA_DIR)):
    ...
    with open(ARGUMENT_DATA_DIR + csv_file, 'rb') as f:
        ...
        for row in reader:
            ...
            #print(vecs[0,:])
            #print(vecs[1,:])
            cur_max1 = np.sum(np.dot((vecs[0,:], dtype='float32'), V_d), dtype='float32') #np.amax(np.dot(vecs[0,:], vecs[2:,:].T))
            cur_max2 = np.sum(np.dot((vecs[1,:], dtype='float32'), V_d), dtype='float32') #np.amax(np.dot(vecs[1,:], vecs[2:,:].T))
